...
class CatGAN_G(LSTMGenerator):
    def __init__(
        ...
        if cfg.model_type == "LSTM":
            ...
            self.lstm = nn.LSTM(
                k_label + embedding_dim, self.hidden_dim, batch_first=True
            )
    ...
    def forward(self, inp, hidden, label=None, need_hidden=False):
        ...
        label_onehot_ex = label_onehot.unsqueeze(1).expand(
            -1, inp.size(1), -1
        )  # batch_size * len * k_label
        label_vec = torch.bmm(
            label_onehot_ex, self.cat_mat.expand(inp.size(0), -1, -1)
        )  # batch_size * len * embed_dim
        emb = torch.cat(
            (emb, label_vec), dim=-1
        )  # batch_sie * len * (k_label + embed_dim)
