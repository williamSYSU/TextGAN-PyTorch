...
class RelGAN_G(LSTMGenerator):
    ...
    #         out, hidden = self.lstm(emb[:, idx, :].unsqueeze(1), hidden)  # out: batch_size * 1 * hidden_dim
    #         all_output = out if idx == 0 else torch.cat((all_output, out), dim=1)
    #
    #     all_output = all_output.contiguous().view(-1, self.hidden_dim)
    #     out = self.lstm2out(all_output)  # (batch_size * seq_len) * vocab_size
    #     out = self.temperature * out  # temperature control
    ...
    def step(self, inp, hidden, temperature=1):
        """
        RelGAN step forward
