...
class CatGAN_C(CNNDiscriminator):
    def __init__(self, k_label, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, gpu=False, dropout=0.25):
        ...
        self.dis_or_clas = 'dis'  # 'dis' or 'clas'
        ...
        self.clas_out2logits = nn.Linear(100 * num_rep, k_label + 1)
    ...
    def forward(self, inp):
        ...
        if self.dis_or_clas == 'dis':
            ...
            pred = self.dis_feature2out(self.dropout(pred))  # batch_size * 100 or batch_size * (100 * num_rep)
            ...
            pred = self.clas_feature2out(self.dropout(pred))  # batch_size * 100 or batch_size * (100 * num_rep)
            logits = self.clas_out2logits(self.dropout(pred.view(inp.size(0), -1))).squeeze(1)  # [batch_size]
