...
class CatGAN_C(CNNDiscriminator):
    def __init__(self, k_label, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, gpu=False, dropout=0.25):
        super(CatGAN_C, self).__init__(embed_dim, vocab_size, clas_filter_sizes, clas_num_filters, padding_idx,
                                       gpu, dropout)

        self.k_label = k_label
        self.embed_dim = embed_dim
        self.max_seq_len = max_seq_len
        self.feature_dim = sum(clas_num_filters)
        self.emb_dim_single = int(embed_dim / num_rep)
        ...
        self.convs = nn.ModuleList([
            nn.Conv2d(1, n, (f, self.emb_dim_single), stride=(1, self.emb_dim_single)) for (n, f) in
            zip(clas_num_filters, clas_filter_sizes)
        ])  # origin
        # self.convs = nn.ModuleList([
        #     nn.Conv2d(1, n, (f, embed_dim)) for (n, f) in zip(dis_num_filters, dis_filter_sizes)
        # ])
        ...
        self.highway = nn.Linear(self.feature_dim, self.feature_dim)
        self.feature2out = nn.Linear(self.feature_dim, 100)  # origin
        self.out2logits = nn.Linear(num_rep * 100, k_label)  # origin
        # self.out2logits = nn.Linear(100, k_label)  # origin
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, inp):
        ...
        pred = self.feature2out(self.dropout(pred))
        logits = self.out2logits(self.dropout(pred.view(inp.size(0), -1))).squeeze(1)  # batch_size * k_label
