...
# Discriminator
class EvoCatGAN_D(CNNDiscriminator):
    def __init__(self, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, gpu=False, dropout=0.25):
        super(EvoCatGAN_D, self).__init__(embed_dim, vocab_size, dis_filter_sizes, dis_num_filters, padding_idx,
                                          gpu, dropout)
...
class EvoCatGAN_C(CNNDiscriminator):
    def __init__(self, k_label, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, gpu=False, dropout=0.25):
        super(EvoCatGAN_C, self).__init__(embed_dim, vocab_size, dis_filter_sizes, dis_num_filters, padding_idx,
                                          gpu, dropout)
    ...
    def forward(self, inp):
        ...
        if self.dis_or_clas == 'dis':
            pred = pred.contiguous().view(-1, self.feature_dim)  # (batch_size * num_rep) * feature_dim
