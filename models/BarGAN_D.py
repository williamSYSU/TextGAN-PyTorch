...
class BarGAN_D(CNNDiscriminator):
    def __init__(self, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, gpu=False, dropout=0.25):
        super(BarGAN_D, self).__init__(embed_dim, vocab_size, dis_filter_sizes, dis_num_filters, padding_idx,
                                       gpu, dropout)
        ...
        # self.out2logits = nn.Linear(100, 2)
    ...
    def forward(self, inp):
        ...
        pred = self.feature2out(self.dropout(pred))  # batch_size * 100
        logits = self.out2logits(pred).squeeze(1)  # batch_size * 2
