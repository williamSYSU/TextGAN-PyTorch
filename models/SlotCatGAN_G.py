...
from models.relational_rnn_slotcat import RelationalMemory


class SlotCatGAN_G(LSTMGenerator):
    def __init__(self, k_label, mem_slots, num_heads, head_size, embedding_dim, hidden_dim, vocab_size, max_seq_len,
        ...
        self.lstm = RelationalMemory(k_label=k_label, mem_slots=1, head_size=head_size,
    ...
    def forward(self, inp, hidden, label=None, need_hidden=False):
        ...
        out, hidden = self.lstm(emb, hidden, label)  # out: batch_size * seq_len * hidden_dim
    ...
    def step(self, inp, hidden, label=None):
        ...
        out, hidden = self.lstm(emb, hidden, label)
