...
# @FileName     : relational_rnn_slotcat.py
...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, k_label, mem_slots, head_size, input_size, num_heads=1, num_blocks=1, forget_bias=1.,
                 input_bias=0.,
                 gate_style='unit', attention_mlp_layers=2, key_size=None, return_all_outputs=False):
        super(RelationalMemory, self).__init__()

        ########## generic parameters for RMC ##########
        self.k_label = k_label  # mem_slot为k_label个，现在的mem_slot表示为每个类别分配几个mem_slot
        self.mem_slots = 1  # 1
    ...
    def initial_state(self, batch_size, trainable=False):
        ...
        Returns:
          init_state: A truncated or padded matrix of size
            (batch_size, self.k_label, self.mem_size).
        """
        init_state = torch.stack([torch.eye(self.k_label) for _ in range(batch_size)])

        # pad the matrix with zeros
        if self.mem_size > self.k_label:
            difference = self.mem_size - self.k_label
            pad = torch.zeros((batch_size, self.k_label, difference))
            init_state = torch.cat([init_state, pad], -1)

        # truncation. take the first 'self.mem_size' components
        elif self.mem_size < self.k_label:
    ...
    def forward_step(self, inputs, memory, label, treat_input_as_matrix=False):
        """
        Forward step of the relational memory core.
        Args:
          inputs: Tensor input.
          memory: Memory output from the previous time step. (batch_size * k_label * mem_size)
          label: Category label, [batch_size]
        ...
        # Select specific memory
        label_onehot = F.one_hot(label, self.k_label).float().unsqueeze(1)  # batch_size * 1 * k_label
        cat_memory = torch.bmm(label_onehot, memory)  # batch_size * 1 * mem_size

        memory_plus_input = torch.cat([cat_memory, inputs_reshape], dim=1)
        ...
        if self.gate_style == 'unit' or self.gate_style == 'memory':
            ...
            input_gate, forget_gate = self.create_gates(inputs_reshape, cat_memory)
            # equation 7 calculation
            next_memory = input_gate * torch.tanh(next_memory)
            next_memory += forget_gate * cat_memory

        output = next_memory.view(next_memory.shape[0], -1)

        update_memory = memory.clone()
        update_memory[torch.arange(memory.size(0)), label, :] = next_memory.squeeze(1)

        return output, update_memory

    def forward(self, inputs, memory, label, treat_input_as_matrix=False):
        ...
        assert type(label) == torch.Tensor, 'missing label Tensor!'

        logit = 0
        logits = []
        # shape[1] is seq_lenth T
        for idx_step in range(inputs.shape[1]):
            logit, memory = self.forward_step(inputs[:, idx_step], memory, label)
