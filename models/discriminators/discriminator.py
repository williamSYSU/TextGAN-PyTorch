...
class GRUDiscriminator(nn.Module):
    def __init__(
        ...
        self.gru = nn.GRU(
            embedding_dim, hidden_dim, num_layers=2, bidirectional=True, dropout=dropout
        )
    ...
    def get_feature(self, inp):
        ...
        out = self.gru2hidden(
            hidden.view(-1, 4 * self.hidden_dim)
...
class CNNClassifier(CNNDiscriminator):
    def __init__(
        ...
        super(CNNClassifier, self).__init__(
            embed_dim, vocab_size, filter_sizes, num_filters, padding_idx, gpu, dropout
        )
    ...
    def forward(self, inp):
        ...
        logits = self.out2logits(self.dropout(pred)).squeeze(
            1
        )  # vanilla, batch_size * k_label
