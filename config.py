...
dataset = (
    "oracle"  # oracle, image_coco, emnlp_news, amazon_app_book, amazon_app_movie, mr15
)
...
vocab_size = (
    5000  # oracle: 5000, coco: 4683, emnlp: 5256, amazon_app_book: 6418, mr15: 6289
)
...
save_root = "save/{}/{}/{}_{}_dt-{}_lt-{}_mt-{}_et-{}_sl{}_temp{}_lfd{}_T{}/".format(
    time.strftime("%Y%m%d"),
    dataset,
    run_model,
    model_type,
    d_type,
    loss_type,
    "+".join([m[:2] for m in mu_type.split()]),
    eval_type,
    max_seq_len,
    temperature,
    lambda_fd,
...
pretrained_gen_path = pretrain_root + "gen_MLE_pretrain_{}_{}_sl{}_sn{}.pt".format(
    run_model, model_type, max_seq_len, samples_num
)
pretrained_dis_path = pretrain_root + "dis_pretrain_{}_{}_sl{}_sn{}.pt".format(
    run_model, model_type, max_seq_len, samples_num
)
pretrained_clas_path = pretrain_root + "clas_pretrain_{}_{}_sl{}_sn{}.pt".format(
    run_model, model_type, max_seq_len, samples_num
)

embedding_root = "pretrain/real_data/" if if_real_data else "pretrain/oracle_data/"
pretrain_embedding_path = embedding_root + "w2v_embedding_size_{}.model".format(
    w2v_embedding_size
)
...
def init_param(opt):
    ...
    # Save path
    save_root = (
        "save/{}/{}/{}_{}_dt-{}_lt-{}_mt-{}_et-{}_sl{}_temp{}_lfd{}_T{}/".format(
            time.strftime("%Y%m%d"),
            dataset,
            run_model,
            model_type,
    ...
    train_data = (
        "dataset/" + dataset + ".txt"
        if if_real_data
        else "pretrain/oracle_data/" + dataset + ".txt"
    )
    test_data = "dataset/testdata/" + dataset + "_test.txt"
    cat_train_data = (
        "dataset/" + dataset + "_cat{}.txt"
        if if_real_data
        else "pretrain/oracle_data/" + dataset + "_cat{}.txt"
    )
    cat_test_data = "dataset/testdata/" + dataset + "_cat{}_test.txt"
    ...
    if max_seq_len == 40:
        ...
        multi_oracle_samples_path = (
            "pretrain/oracle_data/oracle{}_lstm_samples_{}_sl40.pt"
        )
    ...
    ), "Error: eval_b_num = {}, which should be >= n_parent * ADV_d_step ({})!".format(
        eval_b_num, n_parent * ADV_d_step
    )
