...
class FixemGANInstructor(BasicInstructor):
    def __init__(self, opt):
        ...
        if cfg.run_model == "fixemgan":
            labels, train_data = zip(
                *[(0, line) for line in text_file_iterator(cfg.train_data)]
            )
            ...
            labels, train_data = zip(
                *chain(
                    *[
                        [
                            (i, line)
                            for line in text_file_iterator(cfg.cat_train_data.format(i))
                        ]
        ...
        self.train_data_supplier = DataSupplier(
            train_data, labels, w2v, cfg.batch_size, cfg.batches_per_epoch
        )

        self.dis = Discriminator(cfg.discriminator_complexity)
        self.log.info(
            f"discriminator total tranable parameters: {number_of_parameters(self.dis.parameters())}"
        )
        self.gen = Generator(
            cfg.generator_complexity, cfg.noise_size, w2v, cfg.w2v_embedding_size
        )
        self.log.info(
            f"generator total tranable parameters: {number_of_parameters(self.gen.parameters())}"
        )
        ...
        self.G_criterion = GANLoss(
            cfg.loss_type, which_net=None, which_D=None, CUDA=cfg.CUDA
        )
        self.D_criterion = GANLoss(
            cfg.loss_type,
            which_net=None,
            which_D=None,
            target_real_label=0.8,
            target_fake_label=0.2,
            CUDA=cfg.CUDA,
        )
    ...
    def generator_train_one_batch(self):
        ...
        loss = self.G_criterion.G_loss_fixem(
            real_fake_predicts, label_predicts, noise[1], fakes
        )
    ...
    def discriminator_train_one_batch(self, real_vector, labels):
        ...
        loss = self.D_criterion.D_loss_fixem(
            real_fake_predicts, label_predicts[:this_batch_size], labels
        )
        loss.backward()
        self.dis.optimizer.step()

        real_fake_predicts = real_fake_predicts.clone().detach()
        real_fake_predicts = real_fake_predicts.chunk(
            2
        )  # splitting to realand fake parks

        discriminator_acc = float(
            torch.cat((real_fake_predicts[0] > 0.5, real_fake_predicts[1] < 0.5)).mean(
                dtype=float
            )
        )
    ...
    def _run(self):
        for i in trange(cfg.max_epochs):
            for labels, text_vector in tqdm(self.train_data_supplier, leave=False):
                ...
                discriminator_acc = self.discriminator_train_one_batch(
                    text_vector, labels
                )
            ...
            if cfg.run_model == "cat_fixemgan":
                scores = "\n\n".join(
                    [
                        self.cal_metrics_with_label(label_i=label_i, fmt_str=True)
    ...
    def sample_for_metrics_with_label(self, label_i):
        gen_tokens = self.gen.sample(
            cfg.samples_num, 8 * cfg.batch_size, label_i=label_i
        )
        gen_tokens = [sample.split() for sample in gen_tokens]
        gen_tokens_s = self.gen.sample(
            cfg.small_sample_num, 8 * cfg.batch_size, label_i=label_i
        )
        gen_tokens_s = [sample.split() for sample in gen_tokens_s]
        return (
            GenDataIter(gen_tokens),
            gen_tokens,
            gen_tokens_s,
            CatClasDataIter([gen_tokens], label_i),
        )
