...
class SentiGANInstructor(BasicInstructor):
    def __init__(self, opt):
        ...
        # Optimizer
        self.gen_opt_list = [
            optim.Adam(gen.parameters(), lr=cfg.gen_lr) for gen in self.gen_list
        ]
    ...
    def init_model(self):
        ...
        if cfg.clas_pretrain:
            self.log.info(
                "Load  pretrained classifier: {}".format(cfg.pretrained_clas_path)
            )
            self.clas.load_state_dict(
                torch.load(
                    cfg.pretrained_clas_path, map_location="cuda:%d" % cfg.device
                )
            )
    ...
    def _run(self):
        ...
        if not cfg.dis_pretrain:
            ...
            if cfg.if_save and not cfg.if_test:
                ...
                print(
                    "Save pre-trained discriminator: {}".format(cfg.pretrained_dis_path)
                )
        ...
        for adv_epoch in range(cfg.ADV_train_epoch):
            ...
            if self.sig.adv_sig:
                ...
                self.train_discriminator(
                    cfg.ADV_d_step, cfg.ADV_d_epoch, "ADV"
                )  # Discriminator
        ...
        for i in range(cfg.k_label):
            ...
            for step in range(g_step):
                inp, target = GenDataIter.prepare(
                    self.gen_list[i].sample(cfg.batch_size, cfg.batch_size),
                    gpu=cfg.CUDA,
                )

                # ===Train===
                rewards = rollout_func.get_reward(
                    target, cfg.rollout_num, self.dis, current_k=i
                )
        ...
        for step in range(d_step):
            ...
            for epoch in range(d_epoch):
                ...
                d_loss, train_acc = self.train_dis_epoch(
                    self.dis, dis_data.loader, self.dis_criterion, self.dis_opt
                )
    ...
    def cal_metrics_with_label(self, label_i):
        ...
        with torch.no_grad():
            ...
            eval_samples = self.gen_list[label_i].sample(
                cfg.samples_num, 8 * cfg.batch_size
            )
            gen_data = GenDataIter(eval_samples)
            gen_tokens = tensor_to_tokens(eval_samples, self.idx2word_dict)
            gen_tokens_s = tensor_to_tokens(
                self.gen_list[label_i].sample(200, 200), self.idx2word_dict
            )
            clas_data = CatClasDataIter([eval_samples], label_i)

            # Reset metrics
            self.bleu.reset(
                test_text=gen_tokens, real_text=self.test_data_list[label_i].tokens
            )
            self.nll_gen.reset(
                self.gen_list[label_i], self.train_data_list[label_i].loader
            )
    ...
    def _save(self, phase, epoch):
        ...
        for i in range(cfg.k_label):
            ...
            save_sample_path = (
                cfg.save_samples_root
                + "samples_d{}_{}_{:05d}.txt".format(i, phase, epoch)
            )
            samples = self.gen_list[i].sample(cfg.batch_size, cfg.batch_size)
            write_tokens(
                save_sample_path, tensor_to_tokens(samples, self.idx2word_dict)
            )
