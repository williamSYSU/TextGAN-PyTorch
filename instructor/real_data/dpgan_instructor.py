...
class DPGANInstructor(BasicInstructor):
    ...
    def adv_train_generator(self, g_step):
        ...
        dis_count_matrix = (
            torch.Tensor(dis_count_list).unsqueeze(0).repeat(cfg.batch_size, 1)
        )
        if cfg.CUDA:
        ...
        self.log.info(
            "[ADV-GEN]: g_loss = %.4f, %s"
            % (total_g_loss / (g_step * cfg.batch_size), self.cal_metrics(fmt_str=True))
        )
    ...
    def train_discriminator(self, d_step, d_epoch, phase="MLE"):
        ...
        for step in range(d_step):
            ...
            for epoch in range(d_epoch):
                # ===Train===
                pos_reward, neg_reward = self.train_dis_epoch(
                    self.dis, pos_samples, neg_samples, self.dis_opt
                )

            # ===Test===
            self.log.info(
                "[%s-DIS] d_step %d: pos_reward = %.4f, neg_reward = %.4f,"
