...
class DGSANInstructor(BasicInstructor):
    ...
    def _run(self):
        ...
        for adv_epoch in progress:
            ...
            if (
                ...
                self.log.info(
                    "[ADV]: epoch: %d, g_loss = %.4f, %s"
                    % (adv_epoch, g_loss, self.cal_metrics(fmt_str=True))
                )
                if cfg.if_save and not cfg.if_test:
    ...
    def adv_train_generator(self):
        ...
        for (real, fake) in zip(self.oracle_data.loader, gen_data.loader):
            ...
            if cfg.CUDA:
                real_inp, real_tar, fake_inp, fake_tar = (
                    real_inp.cuda(),
                    real_tar.cuda(),
                    fake_inp.cuda(),
                    fake_tar.cuda(),
            ...
            eps = 0
            real_loss = -torch.sum(
                torch.log(1 / (1 + real_old_pred / (real_new_pred + eps) + eps) + eps)
            )
            fake_loss = -torch.sum(
                torch.log(1 / (1 + fake_new_pred / (fake_old_pred + eps) + eps) + eps)
            )
