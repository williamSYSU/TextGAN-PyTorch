...
class CoTInstructor(BasicInstructor):
    def __init__(self, opt):
        ...
        self.dis = Cot_D(
            cfg.gen_embed_dim * 2,
            cfg.gen_hidden_dim * 2,
            cfg.vocab_size,
            cfg.max_seq_len,
            cfg.padding_idx,
            gpu=cfg.CUDA,
        )  # embed_dim and hidden_dim is larger
        self.init_model()
    ...
    def _run(self):
        ...
        for epoch in progress:
            ...
            if epoch % cfg.adv_log_step == 0 or epoch == cfg.ADV_train_epoch - 1:
                self.log.info(
                    "[ADV]: epoch = %d, %s" % (epoch, self.cal_metrics(fmt_str=True))
                )
                if cfg.if_save and not cfg.if_test:
    ...
    def train_mediator(self, cur_epoch, d_step):
        ...
        for step in range(d_step):
            ...
            real = list(self.oracle_data.loader)[
                cur_epoch % len(self.oracle_data.loader)
            ]  # traverse all real data
            real_inp, real_tar = real["input"], real["target"]
            fake_inp, fake_tar = GenDataIter.prepare(
                self.gen.sample(cfg.batch_size, cfg.batch_size), gpu=cfg.CUDA
            )
            if cfg.CUDA:
