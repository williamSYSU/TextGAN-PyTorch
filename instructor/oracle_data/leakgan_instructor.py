...
class LeakGANInstructor(BasicInstructor):
    def __init__(self, opt):
        ...
        self.dis = LeakGAN_D(
            cfg.dis_embed_dim, cfg.vocab_size, cfg.padding_idx, gpu=cfg.CUDA
        )
        self.init_model()
            ...
            if self.sig.pre_sig:
                ...
                if not cfg.dis_pretrain:
                    ...
                    if cfg.if_save and not cfg.if_test:
                        ...
                        print(
                            "Save pre-trained discriminator: {}".format(
    ...
    def adv_train_generator(self, g_step, current_k=0):
        ...
        self.log.info(
            ...
            % (
                adv_mana_loss / g_step,
                adv_work_loss / g_step,
                self.cal_metrics(fmt_str=True),
            )
        )
    ...
    def train_discriminator(self, d_step, d_epoch, phase="MLE"):
        ...
        for step in range(d_step):
            ...
            pos_samples = self.oracle.sample(
                cfg.samples_num, cfg.batch_size
            )  # re-sample the Oracle Data
            ...
            _, eval_acc = self.eval_dis(
                self.dis, dis_eval_data.loader, self.dis_criterion
            )
            self.log.info(
    ...
    def cal_metrics(self, fmt_str=False):
        ...
        if fmt_str:
            return ", ".join(
                [
                    "%s = %s" % (metric.name, metric.get_score())
