...
class GANLoss(nn.Module):
    ...
    def G_loss(self, Dreal, Dfake):
        ...
        if self.which_D == "S":
            ...
            prediction_real = (
                real_tensor if self.loss_mode in ["vanilla"] else fake_tensor
            )
            ...
            raise NotImplementedError(
                "which_D name [%s] is not recognized" % self.which_D
            )
    ...
    def G_loss_fixem(self, real_fake_predicts, label_predicts, target_labels, fakes):
        ...
        real_fake_loss = cfg.real_fake_coeff * self.real_fake_criterion(
            real_fake_predicts, target_fake
        )
        labels_loss = cfg.labels_coeff * self.label_criterion(
            label_predicts, target_labels
        )
    ...
    def D_loss_fixem(self, real_fake_predicts, label_predicts, target_labels):
        target_real = self.get_target_tensor(
            real_fake_predicts.chunk(2)[0], target_is_real=True
        )
        target_fake = self.get_target_tensor(
            real_fake_predicts.chunk(2)[1], target_is_real=False
        )
        target_real_fake = torch.cat((target_real, target_fake))
        real_fake_loss = cfg.real_fake_coeff * self.real_fake_criterion(
            real_fake_predicts, target_real_fake
        )
        labels_loss = cfg.labels_coeff * self.label_criterion(
            label_predicts, target_labels
        )
