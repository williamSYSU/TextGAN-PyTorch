...
def create_logger(name, silent=False, to_disk=False, log_file=None):
    ...
    if to_disk:
        log_file = (
            ...
            else strftime("log/log_%m%d_%H%M.txt", gmtime())
        )
...
def create_oracle():
    ...
    # small
    torch.save(
        oracle.sample(cfg.samples_num // 2, 4 * cfg.batch_size),
...
def get_fixed_temperature(temper, i, N, adapt):
    ...
    if adapt == "no":
        ...
        temper_var_np = 1 + (temper - 1) / np.log(N) * np.log(
            i + 1
        )  # logarithm increase
    elif adapt == "sigmoid":
        temper_var_np = (temper - 1) * 1 / (
            1 + np.exp((N / 2 - i) * 20 / N)
        ) + 1  # sigmoid increase
    elif adapt == "quad":
