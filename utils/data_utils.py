...
def create_multi_oracle(number):
    for i in range(number):
        ...
        torch.save(
            large_samples, cfg.multi_oracle_samples_path.format(i, cfg.samples_num)
        )
        torch.save(
            small_samples, cfg.multi_oracle_samples_path.format(i, cfg.samples_num // 2)
        )
    ...
    for i in range(num):
        while True:
            ...
            if from_a <= groud_truth <= to_b:
                ...
                torch.save(
                    big_samples, "{}_samples_{}.pt".format(prefix, cfg.samples_num)
                )
                torch.save(
                    small_samples,
    ...
    for i in range(num):
        while True:
            ...
            if from_a <= groud_truth <= to_b:
                ...
                torch.save(
                    big_samples,
                    save_path + "{}_samples_{}.pt".format(prefix, cfg.samples_num),
                )
                torch.save(
                    small_samples,
                    save_path + "{}_samples_{}.pt".format(prefix, cfg.samples_num // 2),
                )
                break
